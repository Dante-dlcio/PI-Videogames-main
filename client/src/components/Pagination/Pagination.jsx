...
export function Pagination() {
    ...
    const {videogames, page,videogamesPerPage }= useSelector(state =>state)
    const handleSetPage = x => dispatch(setPage(x))
    const pageButtons = () => [-2, -1, 0, +1, +2]
          .map(x => x + page)
          .filter(x => x >= 0)
          .filter(x => x < videogames.length / videogamesPerPage)
    ...
    return (
      <div className="pagination-buttons">
        <div onClick={() => page > 0 ? dispatch(previousPage()):null}>{"<"}</div>
          {pageButtons()}
        <div onClick={() => page+1 < videogames.length / videogamesPerPage?dispatch(nextPage()):null}>{">"}</div>
      </div>
    );
  }